...
import { defaultFlatConfigNames, resolveAndInstantiateESLint } from './utils/eslint-utils';
...
export default createBuilder(async (options: Schema, context): Promise<BuilderOutput> => {
  try {
    ...
    try {
      ...
      projectName = projectMetadata?.name ?? context.target?.project ?? '<???>';
    } catch {
      /* empty */
    }
    ...
    if (expectedMajor !== null) {
      reportAngularVersionCompatibility(systemRoot, expectedMajor, (message) => {
    ...
    options.cacheLocation = options.cacheLocation ? join(options.cacheLocation, projectName) : null;
    ...
    const { eslint, ESLint } = await resolveAndInstantiateESLint(eslintConfigPath, options);
    ...
    try {
      ...
      if (
        ...
        if (projectRoot) {
          eslintConfigPathForError = resolveESLintConfigPath(projectRoot) ?? '';
        }
...
`);
    ...
    if (options.outputFile) {
      ...
      interpolatedOutputFile = interpolatedOutputFile.replace(/\/+/g, '/').replace(/^\//, '');
    ...
    if ((totalWarnings === 0 || reportOnlyErrors) && totalErrors === 0 && printInfo) {
