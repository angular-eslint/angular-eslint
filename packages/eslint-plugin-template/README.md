...
| [`prefer-let-for-async-pipe`](https://github.com/angular-eslint/angular-eslint/blob/main/packages/eslint-plugin-template/docs/rules/prefer-let-for-async-pipe.md) | Prefer using `@let` to reuse the result of an async pipe when the same expression is subscribed to multiple times in a template view. |  |  |  |  |
...
| [`prefer-style-binding`](https://github.com/angular-eslint/angular-eslint/blob/main/packages/eslint-plugin-template/docs/rules/prefer-style-binding.md) | Suggests using [style] bindings over ngStyle where applicable |  |  |  |  |
