...
export default createESLintRule<Options, MessageIds>({
  ...
  create(
    ...
    const allowedBoundTextPattern = RegExp(
    ...
    return {
      ...
      ...((checkAttributes || checkId) && {
        [`Element > TextAttribute[value=${pLPattern}]`](
      ...
      ...(checkText && {
        [`BoundText, Icu, Text[value=${pLPattern}]`](
          node: StronglyTypedBoundTextOrIcuOrText,
        ) {
...
function isBoundTextAllowed(
  boundTextAllowedPattern: RegExp,
  {
    value: {
      ast: { strings },
    },
  ...
  return boundTextAllowedPattern.test(strings.join('').trim());
}
