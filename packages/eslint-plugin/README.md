...
| [`no-signal-mutable-updates`](https://github.com/angular-eslint/angular-eslint/blob/main/packages/eslint-plugin/docs/rules/no-signal-mutable-updates.md) | Disallow mutable updates to signals. Signals should be read-only and WritableSignals should only be updated via .set() or .update() methods |  |  |  |
