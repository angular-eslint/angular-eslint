  ...
  rules: {
    'require-rule-docs-extension': {
      ...
      create(/** @type {import('eslint').Rule.RuleContext} */ context) {
        return {
          Program(/** @type {import('@typescript-eslint/utils').TSESTree.Program} */ node) {
            const allExports = node.body.filter((s) => s.type === 'ExportNamedDeclaration');
            const ruleDocsExport = allExports.find((s) => {
              ...
              if (decl.type === 'VariableDeclaration' && decl.declarations.length === 1) {
                ...
                return id.type === 'Identifier' && id.name === 'RULE_DOCS_EXTENSION';
              }
              return false;
            });
            ...
            if (!ruleDocsExport) {
              context.report({
                loc: { line: 1, column: 0 },
                message: 'Each rule file must export a named constant `RULE_DOCS_EXTENSION`.',
              });
              return;
            }
            ...
            if (
              ...
              ruleDocsExport.declaration?.declarations[0].id.type !== 'Identifier' ||
              ruleDocsExport.declaration?.declarations[0].id.name !== 'RULE_DOCS_EXTENSION' ||
              ruleDocsExport.declaration?.declarations[0]?.init?.type !== 'ObjectExpression' ||
              ruleDocsExport.declaration?.declarations[0]?.init?.properties?.some(
                (p) =>
                  ...
                  ((p.value?.type !== 'Literal' ||
                    ...
                    (typeof p.value?.value === 'string' && p.value?.value?.trim() === '')) &&
                    // Template literal
                    !(
