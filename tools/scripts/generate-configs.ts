...
const eslintPluginTemplateMaxRuleNameLength = Object.keys(eslintPluginTemplate.rules).reduce(
  (acc, name) => Math.max(acc, name.length),
  0,
);
...
const eslintPluginRuleEntries = Object.entries(eslintPlugin.rules).sort((a, b) =>
  a[0].localeCompare(b[0]),
);
const eslintPluginTemplateRuleEntries = Object.entries(eslintPluginTemplate.rules).sort((a, b) =>
  a[0].localeCompare(b[0]),
);
...
function reducer(
  ...
  const usedSetting: TSESLint.Linter.RuleLevel | TSESLint.Linter.RuleLevelAndOptions =
    settings.errorLevel
      ? settings.errorLevel
      : !recommendation
 ...
 */
async function writeTsBasedConfig(config: string, filePath: string): Promise<void> {
...
`;
  await writeTsBasedConfig(
    tsConfig,
    path.resolve(__dirname, '../../packages/angular-eslint/src/configs/ts-all.ts'),
  );
}
...
`;
  await writeTsBasedConfig(
    tsConfig,
    path.resolve(__dirname, '../../packages/angular-eslint/src/configs/ts-recommended.ts'),
  );
}
...
`;
  await writeTsBasedConfig(
    tsConfig,
    path.resolve(__dirname, '../../packages/angular-eslint/src/configs/template-all.ts'),
  );
}
...
`;
  await writeTsBasedConfig(
    tsConfig,
    path.resolve(__dirname, '../../packages/angular-eslint/src/configs/template-recommended.ts'),
  );
}

async function templatePluginAccessibility() {
  const rules = eslintPluginTemplateRuleEntries
    .filter((entry) => !!entry[1].meta.docs?.description.startsWith('[Accessibility]'))
    .reduce<LinterConfigRules>(
      (config, entry) =>
        reducer('@angular-eslint/template/', config, entry, {
          filterDeprecated: true,
...
`;
  await writeTsBasedConfig(
    tsConfig,
    path.resolve(__dirname, '../../packages/angular-eslint/src/configs/template-accessibility.ts'),
  );
}

function ensureAllRulesAreExportedByPlugin(pluginName: 'eslint-plugin' | 'eslint-plugin-template') {
  readdirSync(path.resolve(__dirname, `../../packages/${pluginName}/src/rules`)).forEach((rule) => {
    const ruleName = rule.replace('.ts', '');
    if (
      pluginName === 'eslint-plugin' &&
      !eslintPlugin.rules[ruleName as keyof typeof eslintPlugin.rules]
    ) {
      throw new Error(`Rule ${ruleName} is not exported by packages/eslint-plugin/src/index.ts`);
    }
    if (
      pluginName === 'eslint-plugin-template' &&
      !eslintPluginTemplate.rules[ruleName as keyof typeof eslintPluginTemplate.rules]
    ) {
      throw new Error(
